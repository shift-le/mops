...
<div class="main">
    ...
    <div class="orddetail-section">
        ...
        <table class="orddetail-table">
            <tr><th>注文ID</th><td>{{ $orderCode }}</td><th>注文日</th><td>{{ $orderDate }}</td></tr>
<tr>
    <th>依頼主名</th>
...
<tr><th>届け先名称</th><td>{{ $header->DELIVERY_NAME ?? '―' }}</td></tr>
<tr><th>配送先住所</th>
<td>{{ $header->DELIVERY_ADDRESS1 ?? '' }} {{ $header->DELIVERY_ADDRESS2 ?? '' }}</td></tr>
<tr><th>配送先電話番号</th><td>{{ $header->DELIVERY_TEL ?? '―' }}</td></tr>
<tr><th>備考</th><td>{{ $header->NOTE ?? '' }}</td></tr>
        </table>
    </div>
